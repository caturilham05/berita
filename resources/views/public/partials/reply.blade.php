	...
	<div style="display: flex; flex-direction: column; justify-content: center; padding: 1rem;">
		...
		<center>
				...
				<div style="color: var(--abu);">
						Jadilah pertama yang komentar disini
				</div>
		</center>
	</div>
@else
	@foreach ($comments as $comment)
		<div class="reply">
			...
			<div style="font-size: 15px; margin: 1rem">{{$comment->comment}}</div>
	    ...
	    <form action="{{route('public.comment.reply', ['content_id' => $content_id,  'comment_id' => $comment->id])}}" method="POST" id="form_{{$comment->id}}" style="margin-top: 1rem;" hidden>
	    	@csrf
